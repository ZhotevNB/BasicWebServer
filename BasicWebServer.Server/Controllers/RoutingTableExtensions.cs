    ...
    public static class RoutingTableExtensions
    {
        ...
        public static IRoutingTable MapControllers(this IRoutingTable routingTable)
        {
            ...
            foreach (var controllerAction in controllerActions)
            {
                ...
                var responseFunction = GetResponseFuncton(controllerAction);

                Method httpMethod = Method.GET;
                var actrionMethodAttribute = controllerAction
                    .GetCustomAttribute<HttpMethotAttribute>();

                if (actrionMethodAttribute != null)
                {
                    httpMethod = actrionMethodAttribute.HttpMethod;
                }
        ...
        private static Func<Request, Response> GetResponseFuncton(MethodInfo controllerAction)
            => request =>
        {
            var controllerInstance = CreateController(controllerAction.DeclaringType, request);
            var parametersValues = GetParameterValues(controllerAction, request);

            return (Response)controllerAction.Invoke(controllerInstance, parametersValues);
        ...
        private static object[] GetParameterValues(MethodInfo controllerAction, Request request)
        {
            ...
            for (int i = 0; i < actionParameters.Length; i++)
            {
                ...
                else
                {
                    ...
                    var parameterProparties = parameter.ParameterType.GetProperties();

                    foreach (var property in parameterProparties)
                    {
